...
const focusedComparison = JSON.parse(await readFile(resolve(root, "results/ace-ag-1.0.15.json"), "utf8"));
const names = { "ace-grid": "Ace Grid Core", "ag-grid": "AG Grid Community", mui: "MUI X Data Grid Community", tanstack: "TanStack Table + Virtual", handsontable: "Handsontable", "react-data-grid": "React Data Grid" };
...
const tableRows = Object.entries(names).map(([id, name]) => {
  ...
  const comparison = result.comparability?.[id] ?? (id === "mui"
    ? { runtimeComparable: false }
    : { runtimeComparable: true });
  if (!comparison.runtimeComparable) {
    return `<tr class="not-comparable"><th><a href="./demo.html?grid=${id}">${name}</a></th><td>${result.versions[versionKeys[id]] ?? "see lockfile"}</td><td>${fmt(bundle ? bundle.gzipBytes / 1024 : null)} KB</td><td colspan="3"><strong>Not comparable:</strong> Community tier forces 100-row pagination</td></tr>`;
  }
...
const focusedRows = [
  ["Ace Grid Core", focusedComparison.summary["ace-grid"]],
  ["AG Grid Community", focusedComparison.summary["ag-grid"]],
].map(([name, sample]) =>
  `<tr><th>${name}</th><td>${fmt(sample.readyMs.median)} ms</td><td>${fmt(sample.readyMs.p95)} ms</td><td>${fmt(sample.scrollSettleMs.median)} ms</td><td>${fmt(sample.scrollSettleMs.p95)} ms</td><td>${fmt(sample.mountedCells.median)}</td></tr>`
).join("\n");
const focusedAce = focusedComparison.summary["ace-grid"];
const focusedAg = focusedComparison.summary["ag-grid"];
const readyLead = percentReduction(focusedAce.readyMs.median, focusedAg.readyMs.median);
const scrollLead = percentReduction(focusedAce.scrollSettleMs.median, focusedAg.scrollSettleMs.median);
...
<section id="results"><div class="section-heading"><div><span class="eyebrow">Measured output</span><h2>Continuous-dataset results</h2></div><p>Medians are useful for orientation, not universal rankings. Runtime depends on browser, hardware, configuration, and product workload.</p></div><div class="table-wrap"><table><thead><tr><th>Implementation</th><th>Version</th><th>JS gzip</th><th>Ready median</th><th>Scroll settle</th><th>Mounted cells</th></tr></thead><tbody>${tableRows}</tbody></table></div><div class="callout"><strong>MUI Community is excluded from runtime comparison:</strong> its documented 100-row pagination limit means it does not expose the same continuous 50,000-row virtualized surface. Its bundle measurement and live fixture remain available. MUI Pro would require a separate commercial-package adapter.</div><p class="environment">${escapeHtml(result.environment.cpu)} · ${result.protocol.browser} · generated ${new Date(result.generatedAt).toLocaleDateString("en-US", { dateStyle: "long" })}</p>
...
<section id="ace-ag-verification"><div class="section-heading"><div><span class="eyebrow">Published-package verification</span><h2>Ace Grid 1.0.15 versus AG Grid, 30 alternating runs</h2></div><p>Each sample uses a fresh browser context. Execution order alternates to reduce warm-machine and ordering bias. Both grids receive the same vertical and horizontal scroll jump.</p></div><div class="table-wrap"><table><thead><tr><th>Implementation</th><th>Ready median</th><th>Ready p95</th><th>Scroll median</th><th>Scroll p95</th><th>Mounted cells</th></tr></thead><tbody>${focusedRows}</tbody></table></div><div class="callout"><strong>Result on this machine:</strong> Ace Grid reached ready ${readyLead}% sooner, settled the scripted scroll ${scrollLead}% sooner, and mounted ${fmt(focusedAg.mountedCells.median - focusedAce.mountedCells.median)} fewer cells. This is evidence for the committed fixture, not a universal fastest-grid claim. <a href="https://github.com/Vitashev/react-data-grid-benchmark/blob/main/results/ace-ag-1.0.15.json">Inspect all 30 raw samples.</a></div></section>
<section class="notes"><article><span class="eyebrow">What this tests</span><h2>A controlled engineering fixture</h2><p>Comparable adapters receive the same generated rows, column count, viewport, row height, two editable columns, sorting, filtering, and continuous virtual-scrolling target. Non-comparable product constraints are shown instead of normalized away.</p></article><article><span class="eyebrow">What this does not prove</span><h2>No universal “fastest grid” claim</h2><p>This does not measure your application, server data model, feature depth, accessibility, support, or total migration cost. Read <a href="https://github.com/Vitashev/react-data-grid-benchmark/blob/main/LIMITATIONS.md">the limitations</a> before citing a number.</p></article></section>
...
await copyFile(resolve(root, "results/ace-ag-1.0.15.json"), resolve(root, "dist/ace-ag-1.0.15.json"));
