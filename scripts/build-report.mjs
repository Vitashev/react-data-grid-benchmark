...
<section id="ace-optimization"><div class="section-heading"><div><span class="eyebrow">Ace Grid optimization candidate</span><h2>A guarded allocation reduction, measured separately</h2></div><p>The candidate reuses empty span metadata and omits an unused row-reorder lookup. It is measured separately because it is not part of the published npm package.</p></div><div class="table-wrap"><table><thead><tr><th>Build</th><th>Ready median</th><th>Scroll settle</th><th>Mounted cells</th></tr></thead><tbody>${aceOptimizationRows}</tbody></table></div><div class="callout"><strong>Result:</strong> this run improved ready median by 2.3% and scroll-settle median by 10.2% with the same mounted DOM. The scroll result is promising, not conclusive, until it is repeated on additional machines. The published-package row above remains the ranking source. <a href="https://github.com/Vitashev/react-data-grid-benchmark/blob/main/results/ace-grid-optimization-candidate.json">Inspect the raw candidate samples.</a></div></section>
