  ...
  .map(([id, name]) => {
    ...
    const cells = [
      rankedMetricCell(
        ...
        "",
        comparable ? interactionRanks.heapReady.get(id) : undefined,
      ),
      rankedMetricCell(
        fmtBytes(sample.afterScrollHeapBytes),
        "",
        comparable ? interactionRanks.heapScroll.get(id) : undefined,
      ),
      rankedMetricCell(
        fmt(sample.longTaskTotalMs),
        "ms",
        comparable ? interactionRanks.longTaskTotal.get(id) : undefined,
      ),
      rankedMetricCell(
        fmt(sample.estimatedFps),
        "FPS",
        comparable ? interactionRanks.fps.get(id) : undefined,
      ),
      rankedMetricCell(
        fmt(sample.droppedFrameCount20ms),
        "",
        comparable ? interactionRanks.droppedFrames.get(id) : undefined,
      ),
    ].join("");
...
const interactionDetailRows = implementations
  .map(([id, name]) => {
    ...
    const note = comparable
      ? ""
      : ' <span class="inline-note">pagination-limited</span>';
    return `<tr${comparable ? "" : ' class="not-comparable"'}><th>${name}${note}</th>${rankedMetricCell(fmtBytes(sample.heapDeltaBytes), "", comparable ? interactionRanks.heapDelta.get(id) : undefined)}${rankedMetricCell(fmt(sample.worstLongTaskMs), "ms", comparable ? interactionRanks.worstLongTask.get(id) : undefined)}${rankedMetricCell(fmt(sample.p95FrameMs), "ms", comparable ? interactionRanks.p95Frame.get(id) : undefined)}${rankedMetricCell(fmt(sample.worstFrameMs), "ms", comparable ? interactionRanks.worstFrame.get(id) : undefined)}</tr>`;
  })
  .join("\n");
...
<section id="results"><div class="section-heading"><div><span class="eyebrow">Measured output</span><h2>Continuous dataset</h2></div><p>Gold, silver, and bronze crowns mark the top three values in each comparable category.</p></div><div class="table-wrap"><table><caption>Production-build measurements on this machine</caption><thead><tr><th>Library</th><th>Version</th><th>JS gzip</th><th>Ready median</th><th>Scroll settle</th><th>Mounted cells</th></tr></thead><tbody>${tableRows}</tbody></table></div><p class="environment">${escapeHtml(result.environment.cpu)} · ${result.protocol.browser} · generated ${new Date(result.generatedAt).toLocaleDateString("en-US", { dateStyle: "long" })}</p></section>
...
function rankedMetricCell(value, unit, rank) {
  ...
  return `<td${rank === 1 ? ' class="winner"' : ""}>${medal}<span>${value}${unit ? ` ${unit}` : ""}</span></td>`;
}

function metricMedal(rank) {
  const names = { 1: "Gold", 2: "Silver", 3: "Bronze" };
  const classes = { 1: "gold", 2: "silver", 3: "bronze" };
  return `<span class="metric-medal metric-medal-${classes[rank]}" role="img" aria-label="${names[rank]} rank" title="${names[rank]} rank">♛</span>`;
}
