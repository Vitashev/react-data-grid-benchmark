...
<section id="results"><div class="section-heading"><div><span class="eyebrow">Measured output</span><h2>Continuous dataset</h2></div><p>Gold, silver, and bronze badges mark the top three values in each comparable category.</p></div><div class="table-wrap"><table><caption>Production-build measurements on this machine</caption><thead><tr><th>Library</th><th>Version</th><th>JS gzip</th><th>Ready median</th><th>Scroll settle</th><th>Mounted cells</th></tr></thead><tbody>${tableRows}</tbody></table></div><p class="environment">${escapeHtml(result.environment.cpu)} · ${result.protocol.browser} · generated ${new Date(result.generatedAt).toLocaleDateString("en-US", { dateStyle: "long" })}</p></section>
...
function metricMedal(rank) {
  ...
  return `<span class="metric-medal metric-medal-${classes[rank]}">${names[rank]}</span>`;
}
